...
* **Circuit Breaker (Fail-Open):** Timeouts estritos na chamada do Groq, derivados do orçamento de latência. **O *timeout* é fixado em 1000ms** (calibrado empiricamente para acomodar variações de rede/TLS no Groq). Caso o motor de compressão estoure essa janela, a exceção é descartada e o proxy direciona o *payload* original de forma limpa para o modelo principal. Em caso de timeout exato ou início lento (cold start), o pior caso de overhead de latência pode atingir 1000ms+, ultrapassando temporariamente o orçamento ideal de overhead de 800ms, mas garantindo zero erro para o desenvolvedor.
...
* **Eficiência de Compressão de Prosa:** Redução mínima de **>30%** nos tokens de entrada correspondentes à linguagem natural (prosa), desconsiderando os blocos de código protegidos.
* **Eficiência de Redução Total do Payload:** Redução de tokens do payload completo (prosa + blocos de código). **Nota de Diluição:** Em prompts pesados em código (code-heavy), a taxa total de redução será naturalmente menor e diluída, pois os blocos de código protegidos permanecem 100% intactos por design.
* **Nota de Cálculo (obrigatória):** Essas taxas devem ser calculadas somente sobre as requisições em que a compressão foi de fato aplicada com sucesso — eventos de *fail-open* devem ser excluídos deste número e contabilizados à parte, conforme a regra da Seção 6.
* **Taxa de Fail-Open (métrica auxiliar):** Percentual de requisições em que o sistema recorreu ao prompt original (por timeout do Groq, rate limits ou reprovação no Sanity Check), monitorado de forma isolada das métricas de eficiência acima.
* **Tolerância de Latência (Overhead):** O hop extra de rede (Proxy -> Groq -> Proxy) tem como meta de regime de operação normal (**warm requests**) um acréscimo máximo de **800 milissegundos** ao tempo total de resposta (*Time to First Token*). Reconhece-se que em cenários de *cold start* ou jitter extremo de rede que alcancem o timeout de 1000ms do Circuit Breaker, este orçamento ideal poderá ser excedido para priorizar o fail-open seguro (entrega garantida do prompt original).
